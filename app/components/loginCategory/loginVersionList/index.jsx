...
class loginVersionList extends React.Component{
    ...
    //     const { refreshLoginVersion, history} = this.props;
    //     sendGetListReq(getLoginVersionByCategoryIdUrl+ id, refreshLoginVersion, "loginVersions", history);
    // }
                    ...
                    <div className="table_opts">
                        {
                            (optsAuths.indexOf('31010241') == -1) ? ''
                                : <Button type="primary"
                                          ...
                                          title="修改"
                                          onClick={()=>{
                                              this.onClickEdit(record.loginVersion)
                                          }}
                                ></Button>
                        }
                        {
    ...
    onClickEdit(orgData){
        // 触发打开登录控制类型模块方法
        const { openLoginVersionModal } = this.props;
        openLoginVersionModal(orgData);
    }

    render(){
        const {  loginVersionList, updateLoginVersionSearch,openLoginVersionModal,optsAuths,  } = this.props;
        return(
